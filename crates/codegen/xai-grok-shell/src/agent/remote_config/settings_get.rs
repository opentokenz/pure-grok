...
pub fn is_eligible(query: &SettingsQuery) -> bool {
    // Fork: the settings getter reads xAI control-plane policy. With the
    // control plane disabled, skip the cache and the network entirely.
...
pub(crate) async fn fetch_settings_live(query: SettingsQuery) -> crate::remote::SettingsFetch {
    // Fork: live settings are control-plane policy; with the control plane
    // disabled report an empty successful read and never touch the network.
    if !crate::control_plane::enabled() {
