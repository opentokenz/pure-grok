...
#[test]
#[serial]
fn feedback_defaults_off_when_control_plane_off() {
    let _fb = EnvGuard::unset("GROK_FEEDBACK_ENABLED");
    let _cp = EnvGuard::unset(crate::control_plane::ENABLE_ENV);
    let cfg = Config::default();
...
#[test]
#[serial]
fn feedback_defaults_on_when_control_plane_on() {
    let _fb = EnvGuard::unset("GROK_FEEDBACK_ENABLED");
    let _cp = EnvGuard::set(crate::control_plane::ENABLE_ENV, "1");
    let cfg = Config::default();
    let r = cfg.feature(Feature::Feedback);
    assert!(
        r.value,
...
#[test]
#[serial]
fn feedback_ignores_remote_when_control_plane_off() {
    let _fb = EnvGuard::unset("GROK_FEEDBACK_ENABLED");
    let _cp = EnvGuard::unset(crate::control_plane::ENABLE_ENV);
    let cfg = Config {
        remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    let r = cfg.feature(Feature::Feedback);
    assert!(!r.value, "remote feedback cannot enable without control plane");
    assert_eq!(r.source, ConfigSource::Default);
}
#[test]
#[serial]
fn feedback_remote_used_when_control_plane_on() {
    let _fb = EnvGuard::unset("GROK_FEEDBACK_ENABLED");
    let _cp = EnvGuard::set(crate::control_plane::ENABLE_ENV, "1");
    let cfg = Config {
        remote_settings: Some(crate::util::config::RemoteSettings {
