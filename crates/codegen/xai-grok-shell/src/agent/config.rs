...
impl Config {
    ...
    pub fn feature(&self, feature: Feature) -> Resolved<bool> {
        let sources = self.feature_sources(feature);
        if feature == Feature::Feedback && !crate::control_plane::enabled() {
            return BoolFlag::env_value(sources.env)
    ...
    pub fn feature_off_reason(&self, feature: Feature) -> Option<String> {
        let resolved = self.feature(feature);
        if resolved.value {
            return None;
        }
        if feature == Feature::Feedback
            && !crate::control_plane::enabled()
            && resolved.source == ConfigSource::Default
        {
            return Some("the default".to_owned());
        }
